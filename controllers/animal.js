...
function pruebas(req, res) {
    res.status(200).send({
        message: ' Probando controlador Animal y accion pruebas',
        user: req.user
    });
};
...
function saveAnimal(req, res){
    ...
    if (params.name) {
        ...
        animal.save((err, animalstored)=>{
          if (err) {
            res.status(500).send({
              ...
              if(!animalstored){
                res.status(404).send({
                    message: 'no se ha guradado el animal'});
        ...
        res.status(500).send({
            message: ' nombre obligatorio animal',
...
function updateAnimal(req, res){
    var animalId = req.params.id;
    var update = req.body;//lo que llega por put
